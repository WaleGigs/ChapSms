...
function getDisplayName(user) {
  ...
  return fullName || user?.username || "ChapsSmS User";
}
...
function getInitials(user) {
  ...
  const username = String(user?.username || user?.email || "C").trim();
...
export default function DashboardTopbar({ onMenuClick }) {
  ...
  const balance = Number(wallet?.balance || 0).toLocaleString("en-NG", {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[var(--border)] bg-[var(--background)]/95 shadow-sm backdrop-blur-xl lg:left-[250px] xl:left-[270px]">
      <div className="mx-auto flex h-16 w-full max-w-[1440px] items-center justify-between gap-2 px-3 min-[375px]:px-4 sm:gap-4 sm:px-6 lg:px-8 xl:px-10">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <button
            type="button"
            onClick={onMenuClick}
            aria-label="Open dashboard navigation"
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--card)] text-[var(--foreground)] transition hover:bg-[var(--muted)] active:scale-95 lg:hidden"
          >
            <Menu size={20} />
          </button>
          ...
          <Link
            href="/buy-number"
            className="truncate text-base font-black tracking-tight text-[var(--foreground)] min-[375px]:text-lg lg:hidden"
          >
        ...
        <div className="flex min-w-0 items-center gap-1.5 min-[375px]:gap-2 sm:gap-3">
          <Link
            ...
            className="flex min-h-10 min-w-0 items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--card)] px-2.5 py-2 transition hover:border-blue-400 hover:bg-[var(--muted)] min-[375px]:px-3 sm:px-4"
          >
            <WalletCards
              size={17}
          ...
          <div ref={profileMenuRef} className="relative shrink-0">
            <button
              ...
              className="flex min-h-10 items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--card)] p-1 transition hover:bg-[var(--muted)] active:scale-[0.98] sm:gap-3 sm:p-1.5 sm:pr-2"
            >
            ...
            <div
              role="menu"
              className={`absolute right-0 top-full z-50 mt-2 w-[calc(100vw-24px)] max-w-64 origin-top-right rounded-2xl border border-[var(--border)] bg-[var(--card)] p-2 shadow-2xl transition duration-150 ${
                profileOpen
