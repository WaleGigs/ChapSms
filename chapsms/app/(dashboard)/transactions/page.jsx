                ...
                {Array.isArray(order.deliveredItems) && order.deliveredItems.length > 0 ? (
                  <div className="mt-4 space-y-2">
                    {order.deliveredItems.map((item, index) => (
                      <div key={`${order.id}-${index}`} className="flex items-center gap-2 rounded-xl bg-[var(--background)] p-3">
                        <p className="min-w-0 flex-1 break-all font-mono text-xs font-semibold">{item}</p>
                        <button type="button" onClick={() => copy(item)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-[var(--border)]">
                          <Copy size={15} />
                        </button>
                      </div>
                    ))}
